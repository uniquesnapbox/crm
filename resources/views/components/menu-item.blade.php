<li id="{{ $accordionId }}" {{ $attributes->merge(['class' => 'accordionItem ' . ($active ? 'openIt' : 'closeIt')]) }}>
    ...
    @else
        <button type="button"
            class="nav-item text-lightest f-15 sidebar-text-color accordionItemHeading {{ $active ? 'active' : '' }}"
        ...
        <div id="{{ $accordionId }}-panel" class="accordionItemPanel">
            {{ $slot }}
        </div>
    @endif
