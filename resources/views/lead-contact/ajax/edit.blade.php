...
$addPermission = user()->permission('add_lead');
        ...
        <x-form id="save-lead-data-form" method="PUT">
            <div class="bg-white rounded p-20">
                <h4 class="mb-4 f-21 font-weight-normal text-capitalize border-bottom-grey pb-3">
                    @lang('modules.leadContact.updateTitle')
                </h4>

                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <x-forms.text :fieldLabel="__('app.name')" fieldName="client_name"
                                      fieldId="client_name" fieldRequired="true"
                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="source_id" id="source_id" data-live-search="true">
                                    ...
                                    @foreach ($sources as $source)
                                        <option value="{{ $source->id }}" @selected($leadContact->source_id == $source->id)>
                                            {{ $source->type }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="col-md-12">
                        <div class="form-group my-3">
                            ...
                            <textarea class="form-control" name="address" id="address" rows="2"
                                      placeholder="@lang('placeholders.address')">{{ $leadContact->address }}</textarea>
                ...
                <x-forms.custom-field :fields="$fields" :model="$leadContact" class="col-md-12 mt-2"></x-forms.custom-field>
...
<script>
    $(document).ready(function() {
        ...
        function saveLead(formAction, buttonSelector) {
            let url = "{{ route('lead-contact.update', [$leadContact->id]) }}";
