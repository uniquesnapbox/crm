...
app.get("/qr", requireApiKey, (req, res) => {
  ...
  const run = async () => {
    if (forceRefresh) {
      manager
        .reconnect(sessionKey, "manual_qr_refresh")
        .catch((error) => {
          logger.error("Manual QR refresh failed", {
            sessionKey,
            error: error.message
          });
        });
    ...
    const status = manager.getStatus(sessionKey);
    const qr = manager.getQr(sessionKey);
    const qrUpdatedAt = manager.getQrUpdatedAt(sessionKey);
...
async function bootstrap() {
  ...
  // Initialize sessions in background so health/API remains reachable.
  setImmediate(async () => {
    try {
